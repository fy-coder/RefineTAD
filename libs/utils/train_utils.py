        ...
        with torch.no_grad():
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    print(output[vid_idx]['segments'][:, 0])
                    exit()
...
def valid_one_epoch_all(
    ...
    if not refine:
        if evaluator is not None:
            # if ext_score_file is not None and isinstance(ext_score_file, str):
            #     TAD_results = postprocess_results(TAD_results, ext_score_file)
            # call the evaluator
            _, mAP, _ = evaluator.evaluate(TAD_results, verbose=True)
        else:
            mAP = 0.0
        return mAP
    ...
    base_results = dict()
    for iter_idx, video_id in enumerate(TAD_results['video-id']):
        base_results[video_id] = {
            'segments': [],
            'label': [],
            'score': []
        }

    for iter_idx, (video_id, t_start, t_end, label, score) in enumerate(
            zip(TAD_results['video-id'], TAD_results['t-start'],
                TAD_results['t-end'], TAD_results['label'], TAD_results['score'])):
        base_results[video_id]['segments'].append([t_start, t_end])
        base_results[video_id]['label'].append(label)
        base_results[video_id]['score'].append(score)

    for iter_idx, video_list in enumerate(val_loader, 0):
        # forward the model (wo. grad)
        with torch.no_grad():
            video_id = video_list[0]['video_id']
            if video_id not in base_results.keys():
                continue
            output = ref_model(video_list, base_results[video_id])
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    results['label'].append(output[vid_idx]['labels'])
                    results['score'].append(output[vid_idx]['scores'])
                    # print(output[vid_idx]['segments'][:, 0].shape)
                    # exit()
